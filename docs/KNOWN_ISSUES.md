...
- **WAF hits on non-resolving subdomains** [firewall_threat, dns_security, cloudflare]: WAF hits or DNS queries on subdomains that don't resolve publicly (tracker, nas, qnap, unifi, zoneminder, webmail, ntopng, paperless, ipmi, haproxy, krusty, dahua, nextcloud, wiki, photos, api, bookstack, etc.) are not findings. If it doesn't resolve, there's nothing to protect. Do NOT recommend the user verify these — call check_public_dns() yourself and drop any that return NXDOMAIN.
...
- **Amazon device 28:24:c9:60:1e:1c** [firewall_threat, wireless, infrastructure]: OUI = Amazon Technologies Inc. (Kindle/Fire tablet, udhcp 1.34.1 BusyBox client). Authenticates to Elephant SSID via WPA-PSK on VLAN 1. Benign traffic pattern (mDNS, content sync, zero suspicious outbound). Likely unregistered household Kindle or Fire tablet — pending identification by owner. Do NOT flag as a threat. DHCP churn (.200-.244 range) is from lack of static reservation, not multiple devices.
