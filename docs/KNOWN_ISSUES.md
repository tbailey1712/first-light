...
- **Subdomain enumeration** [firewall_threat, dns_security, cloudflare]: Random-string, hex-string, SRV probes, port-encoded probes (_8123._https.ha, _8006._https.pve), IP-encoded rebind probes (172.67.x.x_*), and numeric-prefix enumeration against mcducklabs.com are automated DNS recon returning NXDOMAIN. Not actionable unless a probe hits a live record (verify with check_public_dns). Do NOT classify as "coordinated campaign" or "critical" — this is background internet noise.
- **WAF hits on non-resolving subdomains** [firewall_threat, cloudflare]: Low-volume WAF hits on subdomains that don't resolve publicly (tracker, nas, qnap, unifi, zoneminder, webmail, ntopng, paperless, ipmi, haproxy, krusty, dahua, etc.) are not findings. If it doesn't resolve, there's nothing to protect.
